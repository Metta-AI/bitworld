...
proc initBot*(paths: Paths = defaultPaths(), masterSeed: int64 = 0): Bot =
  ...
  result.motion = Motion(frameAdvance: 1)
  ...
  result.llmVoting = initLlmVotingState()
  # Process-lifetime LLM counters. `LlmState` is zero-initialised by
  # default; we just stamp `layerActiveAckTick = -1` as "not ack'd yet"
  # (Python wrapper flips this when modulabot_enable_llm fires).
  result.llm = LlmState(layerActiveAckTick: -1)
...
proc resetRoundState*(bot: var Bot) =
  ...
  # LLM state: reset per-meeting bookkeeping but preserve the
  # `enabled` flag, which is a process-lifetime gate flipped by the
  # FFI at load time.
  ...
  if bot.percep.interstitial:
    ...
    if bot.voting.active:
      when defined(modTalksLlm):
        # LLM voting state machine. Runs after parseVotingScreen so
        # `bot.voting.chatLines` is fresh. `tickLlmVoting` no-ops if
        # `llmVoting.enabled` is false (Python wrapper hasn't called
        # modulabot_enable_llm) or if we've already committed.
        tickLlmVoting(bot)
