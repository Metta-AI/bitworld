...
- Player: `http://localhost:2000/client/player.html?address=ws://localhost:2000/player`
- Global viewer: `http://localhost:2000/client/global.html?address=ws://localhost:2000/global`
- Rewards: `http://localhost:2000/client/rewards.html?address=ws://localhost:2000/reward`

The player client connects to `/player`, the global viewer connects to
`/global`, and the rewards viewer connects to `/reward`.
...
```sh
nim r tools/quick_player nottoodumb 1 --address:localhost --port:2000
```
...
- `--players:N`: Alternative way to set the number of bots.
...
```sh
cd /Users/me/p/bitworld
nim r tools/quick_run among_them 2000 --players:4 --address:0.0.0.0
```

If the port is omitted, `quick_run` chooses a random port between 5000 and
10000.

```sh
nim r tools/quick_run among_them --players:2
```

You can save a replay while using `quick_run`:

```sh
nim r tools/quick_run among_them 2000 --players:2 --save-replay:among_them.replay
```
...
```text
http://localhost:2000/client/global.html?address=ws://localhost:2000/global
```
