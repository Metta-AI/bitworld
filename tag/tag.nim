...
proc configureMatch(sim: var SimServer, controlledPlayers: int) =
...
proc applyTag(sim: var SimServer) =
  ...
  for i in 0 ..< sim.players.len:
    ...
    if rectsOverlap(t.x - 1, t.y - 1, t.sprite.width + 2, t.sprite.height + 2,
      ...
      sim.players[taggerIndex].reward += 50
...
proc awardProximityReward(sim: var SimServer) =
  if sim.players.len < 2:
    return
  ...
  for i in 0 ..< sim.players.len:
    ...
    if manhattan <= TileSize * 10:
      inc sim.players[taggerIndex].reward
...
proc buildRewardPacket(sim: SimServer): string =
  for i in 0 ..< sim.players.len:
    ...
    result.add($i)
    result.add(" ")
    result.add($sim.players[i].reward)
    result.add("\n")
      ...
      withLock appState.lock:
        ...
        if appState.resetRequested:
          ...
          if appState.playerIndices.len > 0:
            sim.configureMatch(appState.playerIndices.len)
