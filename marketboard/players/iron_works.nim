...
proc decide*(bot: var BotState, state: GameState): uint8 =
  ...
  of SelectSellItem:
    if p.state != "AtSellStall":
      ...
      bot.ticksInPhase = 0
      return 0
    let gearCursor = firstGearSellCursor(p)
    if gearCursor < 0:
      bot.phase = ExitSell
      bot.ticksInPhase = 0
      return 0
  ...
  of SetPrice:
    if p.state != "AtSellStall":
      ...
      bot.ticksInPhase = 0
      return 0
    var itemName = "LeatherHat"
    for gi in GearItemNames:
      if p.inv.itemCount(gi) > 0:
        itemName = gi
        break
    ...
    if bot.cancelCycles > 0:
      targetPrice = max(1, targetPrice - bot.cancelCycles * 2)
