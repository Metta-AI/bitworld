# marketboard

- rough draft


"Cooperative/competitive social dilemma games, non-zero-sum. Make short term betrayal possible and beneficial, but hinders long term success."

- the idea is to draw inspiration from what I find make ffxiv and EVE Online fun
- could go with either a space theme or a fantasy theme (fantasy might be cooler? I think we already have a lot of space / future themed games?)

- there's a central marketboard / trading hub. think ffxiv marketboard or EVE Online trading hub
- reallistic economy, this is the core part of the game (also mimics real life)
  - trades have to have 2 sides to the trade. a buyer and a seller. I think for simplicity, we should mimic FFXIV style of only sell orders with a limited # of slots (eg: 4, 8, maybe 16) and a fixed quantity per slot (say a stack of 1-99 for a specific item)
- there are 2 roles, 'gatherers' and 'crafters'. 
  - ffxiv has 3 gathering roles, and 8 crafting classes. for this game, let's simplify to just gatherer or crafter.
- there has to be some sort of a 'cost' to switching.
  - perhaps you have to pay money to switch, eg: in EVE you'd have to buy different ships for a different activity
  - or maybe you get a level reset? or maybe we could do ffxiv style where say you level gathering to 5, switch to crafter which is at 1, level crafter up, then if you switch back to gatherer, you resume lvl 5? but there's obviously a cost to splitting your effort across classes, it's better to specialize.
- the clear "social dilemma" would be real market situations like price gouging or hoarding.
  - eg: if there was 1 gatherer and 3 crafters, the gatherer could hoard all the base materials and price gouge all the crafters.
 

- gatherers get raw materials that crafters need.
  - oh, but crafters need to make something gatherers need..
  - how about crafters can make gear items that gatherers can use to gather more resources?
  - maybe have like 5 item slots, a hat, shirt, gloves, pants and shoes?
  - so gatherers want better gear from crafters to gather more, and crafters want materials from gatherers.
- maybe we could also have 2 or 3 tiers of resources? perhaps tier 2 requires a full set of tier 1 gear, and tier 3 requires a full set of tier 2 gear at minimum to gather?
  - higher level materials could craft better equipment.
- keep materials simple: maybe 3 tiers of wood, and 3 tiers of ore (stone, copper, iron)

- I'd like to avoid menus. maybe the marketboard could have some way of interfacing where agents stand on squares to decide how much to buy/sell?
  - should ponder more on this.
